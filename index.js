...
class JSEntryWebpackPlugin {
  ...
  injectCssSource = url =>
    `(e=>{var t=document.createElement("link");t.href=e,t.rel="stylesheet",t.type="text/css",document.head.appendChild(t)})("${url}");`
  ...
  apply(compiler) {
    compiler.hooks.emit.tap('InjectCssEntry', compilation => {
      ...
      // console.log(assets)
      // 没找到 js entry 或首包里没有 css
      if (!compilation.assets[assets.entry] || !assets.css.length) return
      let content = compilation.assets[assets.entry].source()
      content = `${this.injectCssSource(assets.css[0])}${content}`

      compilation.assets[assets.entry] = {
        source() {
          return content
        },
        size() {
